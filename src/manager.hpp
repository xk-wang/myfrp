...
public:
    ...
    // frps的工作线程 可以来告知端口号
    Manager(int port, int ffd1, int ffd2);
    ~Manager();
...
Manager::Manager(int ffd1, int ffd2): fd1(ffd1), fd2(ffd2),
        ...
        backward_read_idx(0), backward_write_idx(0), port(-1){
...
void* Manager::start_frpc_routine(void* arg){
...
void* Manager::start_frps_routine(void* arg){
    Manager* manager = (Manager*)arg;
    int epollfd = manager->epollfd;
    // fd1是client，fd2是frpc
    int fd1 = manager->fd1, fd2 = manager->fd2;
